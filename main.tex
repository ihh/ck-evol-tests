...
\section{Alternative: sampling from the model}
\seclabel{ModelSampling}

An alternative to using a TKF-based proposal distribution is to
sample directly from the model $M$ itself.
Given an ancestral sequence $A$ and times $\ell_1,\ell_2$:
\begin{enumerate}
\item Sample $C \sim M(C|A,\ell_1+\ell_2)$ to get a descendant that $M$ considers likely.
\item Sample $N$ intermediates $B_n \sim M(B|A,\ell_1)$ by forward simulation.
\item Estimate $Z$ by simple Monte Carlo:
\begin{equation}
Z(C|A,\ell_1,\ell_2) \simeq \frac{1}{N} \sum_{n=1}^{N} M(C|B_n,\ell_2)
\eqnlabel{ModelMonteCarlo}
\end{equation}
\item Compare to $M(C|A,\ell_1+\ell_2)$ as per \eqnref{ChapmanKolmogorov}.
\end{enumerate}
Since $B_n$ is drawn from $M(B|A,\ell_1)$, the importance weight
$M(B|A,\ell_1)/q(B)$ reduces to unity, and the estimator is simply an average of $M(C|B_n,\ell_2)$.

One could also use a TKF-based (or other) proposal $q(B|A,C,\ell_1,\ell_2)$ in step~2
and restore the importance weights as in \eqnref{ImportanceSample},
combining the model-sampled $C$ with a variance-reduced estimate of $Z$.

\subsection{Comparison of the two approaches}

The two approaches---using a TKF proposal (\secref{ImportanceSample}) vs.\ sampling from $M$---differ
in several respects.

\paragraph{Variance.}
The TKF proposal $q(B|A,C,\ell_1,\ell_2)$ conditions on both $A$ and~$C$,
so the sampled intermediates $B$ are concentrated in the region where
$M(B|A,\ell_1)M(C|B,\ell_2)$ is large,
yielding low-variance importance weights.
By contrast, sampling $B \sim M(B|A,\ell_1)$ ignores $C$ entirely:
many samples will produce intermediates for which $M(C|B,\ell_2)$ is negligible,
wasting effort. This problem worsens as sequences grow longer or evolutionary distances increase.

\paragraph{Requirements on $M$.}
The model-sampling approach requires that $M$ can both generate sequences
(sample from $M(B|A,\ell_1)$ and $M(C|A,\ell_1+\ell_2)$) and
evaluate conditional likelihoods ($M(C|B,\ell_2)$).
The TKF proposal approach requires only that $M$ can evaluate likelihoods,
not that it can sample; the sampling is handled by the TKF machinery.
For neural models that are naturally generative (e.g.\ autoregressive sequence models),
the model-sampling approach may be more natural;
for models where likelihood evaluation is easy but sampling is expensive, the TKF approach is preferable.

\paragraph{Choice of $C$.}
In the model-sampling approach, $C$ is drawn from $M(C|A,\ell_1+\ell_2)$,
so we automatically test Chapman-Kolmogorov at points where $M$ places mass.
In the TKF approach, $C$ can be chosen freely---e.g.\ from training data
or from the TKF model---and need not be representative of $M$'s distribution.
For computing a KL divergence (\secref{KullbackLeibler}), sampling $C$ from $M$ is appropriate
since the expectation is over $M(C|A,\ell_1+\ell_2)$.

\paragraph{Implementation complexity.}
The model-sampling approach avoids the need for TKF parameterization and the
triad HMM machinery of \secref{Algorithms}, requiring only forward simulation
and likelihood evaluation under $M$.
The TKF approach requires implementing the pair and triad HMMs,
null state elimination, and stochastic traceback,
but rewards this investment with substantially lower estimator variance.

\paragraph{Hybrid approach.}
A practical middle ground is to sample $C$ from $M$ (to focus on likely descendants)
but use a TKF proposal for the $B$ intermediates (to reduce variance in the $Z$ estimate).
...
The list is sorted by decreasing $p_k$, and can be filtered for the top $K$ only.
...
as follows
\begin{equation}
Z(C|A,\ell_1,\ell_2) \simeq \frac{1}{N} \sum_k c_k \frac{M(B_k|A,\ell_1) M(C|B_k,\ell_2)}{p_k}
\eqnlabel{ImportanceSample}
\end{equation}

This can then be compared to $M(C|A,\ell_1+\ell_2)$ as per \eqnref{ChapmanKolmogorov}.

Alternatively, we can obtain a strict lower bound on $Z(C|A,\ell_1,\ell_2)$ by summing over the top $K$
samples without importance weights (the expensive calculation of which can be disabled using the {\tt --no-posteriors} option)
\begin{equation}
Z(C|A,\ell_1,\ell_2) \geq \sum_k M(B_k|A,\ell_1) M(C|B_k,\ell_2)
\eqnlabel{LowerBound}
\end{equation}
...
\[
\jointpairhmm(\params,\ell) = (\states,\alphabet,{\bf t},{\bf e},\statetype)
\]
...
Here $\alphabet$ denotes the sequence alphabet, i.e. the index space of the substitution model $(\submat,\eqm)$.
...
\begin{eqnarray*}
   ...
   \left\{
   \begin{array}{ll}
   \eqm_{\omega} \exp(\submat \ell)_{\omega \omega'} & \mbox{if $\omega,\omega' \in \alphabet$}
   \\ 0 & \mbox{otherwise}
   \end{array}
   \right. \\
...
\[
\conditionalpairhmm(\params,\ell) = (\states,\alphabet,{\bf t},{\bf e},\statetype)
\]
where $\params = (\insrate,\delrate,\fragext,\submat,\eqm)$.
...
Since it's conditionally normalized, it is technically a finite-state transducer rather than a Pair HMM.
...
\begin{eqnarray*}
   ...
   \left\{
   \begin{array}{ll}
   \exp(\submat \ell)_{\omega \omega'} & \mbox{if $\omega,\omega' \in \alphabet$}
   \\ 0 & \mbox{otherwise}
   \end{array}
   \right. \\
   e_\ins(\omega,\omega') & = & \eqm_{\omega'} \\
...
Technically it is a three-sequence HMM, but we will treat $B$ as unobserved.
As such, we represent it as a conditional Pair HMM (or, more precisely, a finite-state transducer)
that inputs $A$ and outputs $C$.
...
e_k(\omega,\omega') & = &
   \left\{
   \begin{array}{ll}
   \exp(\submat (\ell_1 + \ell_2))_{\omega \omega'} & \mbox{if $k = \matmat$ and $\omega,\omega' \in \alphabet$} \\
   \eqm_{\omega'} & \mbox{if $k \in \{ \stains, \matins, \insmat, \insins \}$, $\omega=\epsilon$, and $\omega' \in \alphabet$} \\
   1 & \mbox{if $k \in \{ \matdel, \delsta, \delmat, \delins, \deldel \}$, $\omega \in \alphabet$, and $\omega' = \epsilon$} \\
   0 & \mbox{otherwise}
   \end{array}
   \right. \\
 \statetype(k) & = & \left\{ \begin{array}{ll}
 \sta\ & \mbox{if $k = \triadstart$} \\
 \mat\ & \mbox{if $k = \matmat$} \\
 \ins\ & \mbox{if $k \in \{ \stains, \matins, \insmat, \insins \}$} \\
 \del\ & \mbox{if $k \in \{ \matdel, \delsta, \delmat, \delins, \deldel \}$} \\
 \nul\ & \mbox{if $k = \insdel$} \\
...
Let $A,C \in \alphabet^\ast$ be sequences on an endpoint-conditioned trajectory
...
can be expressed in closed form for the special case where the model ($M^\abcsup$) is the triad HMM
...
\algrequire{$A,C \in \alphabet^\ast$, $\params = (\insrate,\delrate,\fragext,\submat,\eqm)$}
...
\State $M^\abcsup \gets \triadhmm(\params,\ell_1,\ell_2)$
\State $M^\acsup \gets \eliminatestates\left(M^\abcsup, \{ \insdel \}\right)$ \Comment{See \sref{ClosedFormStateElim}}
...
\State $p^\acsup \gets F^\acsup(\len_A,\len_C,\triadend)$  \Comment{$\forwardlikelihood\left(A,C,M^\acsup\right)$}
\State \For{$n = 1$ to $N$}
\State $\statepath^\acsup \gets \forwardtraceback\left(M^\acsup,F^\acsup\right)$
\State $\statepath^\abcsup \gets \restorestates\left(M^\abcsup,\{\insdel\},\statepath^\acsup\right)$ \Comment{See \sref{ClosedFormStateElim}}
\State $B_n \gets \epsilon$
\State $i \gets \len_A$
\State $j \gets \len_C$
\While{$\statepath^\abcsup$ is nonempty}
  \State $k \gets \pop\left(\statepath^\abcsup\right)$
  \State \If{$k \in \{ \matmat, \insmat, \matdel, \insdel \}$} \Comment{Residue observed at $B$?}
  \State \If{$k = \matmat$} \Comment{$B$ aligned to $A$ and $C$}
    \State For $\omega \in \alphabet:\ q_\omega \gets U_{A_i, \omega} V_{\omega, C_j}$
  \ElsIf{$k = \insmat$} \Comment{$B$ aligned to $C$ only}
    \State For $\omega \in \alphabet:\ q_\omega \gets \eqm_\omega V_{\omega, C_j}$
  \ElsIf{$k = \matdel$} \Comment{$B$ aligned to $A$ only}
    \State For $\omega \in \alphabet:\ q_\omega \gets U_{A_i, \omega}$
...
\State $p_n \gets p^\absup p^\bcsup / p^\acsup$  \Comment{$p_n = \prob{B_n|A,C,\theta,\ell_1,\ell_2}$}
\EndFor
\\
\Return $[ (B_n, p_n): 1 \leq n \leq N ]$
\Comment{Can cache duplicates; see \sref{DuplicateSamples}}
\end{algorithmic}
\end{algorithm}
